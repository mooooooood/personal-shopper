...
**从家犬功能之前的版本升级时，必须覆盖完整 `app/` 并重启服务：共享家犬的 20 秒驱赶、HTTP 接口和 v7 存档都由 Python 后端执行，只更新模板和 static 不够。** 当前页面移除 Pause、Release、Sync 按钮及左上角计分板，Watch 初始不选中，保留 Let dog out 按钮及倒计时。原有兔子窝、地图上的八个席位和篮子、套索拉回、鹰狼事件和 WhatsApp 联系方式保留；没有新增依赖。本次计分板调整不改变存档格式或数据。下列命令更新完整代码和旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
