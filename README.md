...
现在整个首页都是一个可以互动的二维兔子小牧场。访客可以看白兔跳动、结伴、生宝宝，也可以放胡萝卜、用网抓兔子、把兔子放回草地。画面上只保留悬浮工具与统计；点 **About & contact** 才打开简短的个人介绍和联系方式。希望这片小草地，让初次见面多一点轻松。

## 当前网站内容

- **个人介绍**：第一人称说明一人经营的工作方式，放在随时可以打开或关闭的信件面板里。
- **兔子小牧场**：Canvas 绘制草地、池塘和白兔；自动跳动、结伴繁殖，支持投喂、捕捉、放回、暂停与重新开始。
- **采购范围**：八个示例品类与“Something else?”入口，支持按链接、参考图片或描述提出需求。
- **满屏游戏**：去掉原来的网页页头、侧栏和页脚；草地随手机或电脑的屏幕尺寸调整，兔子保持正常比例。
...
- **Catch / Release**：选 Catch 后点兔子收进篮子，Release 每次放回一只。不会伤害兔子。
- **Pause / Play**：暂停或继续；暂停时仍能投喂和抓放兔子。
- **New meadow**：重新开一片草地，回到 12 只成年兔子。
- **About & contact / Help**：打开个人介绍和联系卡片，或查看玩法；打开面板时牧场暂停，关闭后恢复原先状态。
- **键盘**：聚焦地图后用方向键移动光圈，Enter 或空格使用工具；W / C / N 切换看兔子、胡萝卜和捕捉。

成年兔子会自动寻找伙伴，以爱心表现配对，随后出现小兔。小兔约 30 秒长大；开始游玩后通常约 12–20 秒能看到第一批出生。草地和篮子合计最多 60 只，满员后暂停繁殖。

牧场全部在访客自己的浏览器运行，以约 30 帧/秒为绘制上限，背景绘制后复用，仅屏幕尺寸变化时重绘。切到后台或打开信息面板会停止更新，系统设置“减少动态效果”时默认暂停。调整窗口或旋转手机会重新布局地图，并保留兔子、篮子和出生计数。本次没有增加服务器依赖、数据库表或定时任务。

每次打开或刷新页面都是新的牧场，不保存兔子进度；不同访客的兔子互不影响。SQLite 仍只保存网站介绍、联系方式与产品资料。已有服务器更新 `app/` 并重启服务即可使用牧场，不需要更改 Nginx 或重新安装依赖。
...
Python 测试覆盖首页、所有详情页、404、数据库及私有文件不可访问、静态资源类型、健康检查、安全响应头、模板转义、自定义域名站点地图，以及数据库首次导入、持久化、无效数据拒绝、事务回滚、备份独立性和默认文案迁移时的自定义资料保留。

可选：使用 Node.js 22.7 或更新版本运行兔子模型的开发测试（生产服务器不需要安装 Node.js）：

```sh
node --test tests/meadow-model.test.mjs
```

模型测试覆盖出生和长大、捕捉打断配对、篮子计入容量、池塘绕行、胡萝卜吸引与过期、调整地图尺寸后状态保留，以及暂停和过大时间间隔的处理。
