...
现在整个首页都是一个所有访客共同游玩的二维兔子牧场。大家看到同一群兔子跳动、结伴、生出不同毛色的宝宝，也可以放胡萝卜、像牛仔一样用套索套住兔子并拉回共享篮子，再把兔子放回草地。老鹰和灰狼偶尔来访，拉回途中也可能把兔子叼走，只有进了篮子才安全。一个人的操作会同步给其他访客，刷新或重新进入也会保留牧场进度。画面上保留悬浮工具与统计；右上角只有一个醒目、间歇轻跳的 **About & contact** 按钮，打开简短的个人介绍后，可直接通过 **WhatsApp +86 15927146828** 联系经营者。希望这片小草地，让初次见面多一点轻松。
...
- **Pause / Play**：只暂停自己的画面；暂停时禁用投喂、套索和放回，并停止拉动。切到后台或打开信息面板也停止拉动；已经抛出的套索仍按公共时钟落地。回到活动画面后，未过期的套索会自动继续拉回；绳子的剩余时间仍由公共牧场决定。Play 重新同步到最新状态，其他访客的游玩不会被你的暂停影响。
- **Sync**：立即获取共享牧场的最新状态，不会清空或重置世界。
- **About & contact**：右上角唯一的业务入口，打开个人介绍和 WhatsApp 联系卡片；打开面板时停止连续动画，并降低同步频率，关闭后按原来的播放状态恢复。入口用较醒目的像素色块和间歇轻跳提醒访客，悬停、聚焦或打开面板时停止跳动。
...
兔子使用手工排列的像素精灵：有限色板描绘不同毛色、长耳朵、前后腿和眼睛高光，保留清晰的方格轮廓。静止帧和四个跳跃帧表现预蹲、蹬地、腾空与落地，并有眨眼变化；每种毛色和姿态首次绘制后缓存复用。老鹰、灰狼和胡萝卜也使用同一套像素绘制方式，放大时关闭平滑处理，不下载模型或大图。显示位置仍由共享快照决定，动画不会自行移动、繁殖或改变兔子。

草地采用分层像素场景：深浅绿树冠围住明亮的块状草坪，点缀少量小花；阶梯状池塘搭配玉绿色水面、石头和浅土色岸边，奶油色篱笆、土路与兔子窝保持统一色板。兔子随地图远近轻微缩放，点击位置和套索也按相同尺度对齐，服务器坐标不变。草地纹理和边缘前景只在窗口尺寸变化时重新绘制并缓存；水面仅有四组缓慢变化的像素水纹。仍保持约 30 帧上限，暂停、后台省电与减少动态效果的设置继续生效，没有新增图片下载、三维引擎或前后端依赖。
...
这些逻辑都运行在现有 FastAPI 服务中，无新增服务、框架、依赖或 Nginx 配置。**Uvicorn 必须保持 1 个 worker**，不能通过多进程或多实例同时写同一牧场。本次 WhatsApp 更新涉及模板、后端链接生成、数据库迁移与初始化资料；已有服务器必须更新完整 `app/` 和 `data/site.json` 后重启，不能只换 HTML 和静态文件。
...
数据库位于代码目录之外，正常拉取代码不会覆盖它；JSON 主要用于第一次初始化，已有资料的特定升级通过下文的一次性迁移完成。本次需要拉取包含完整 `app/` 与 `data/site.json` 的版本并重启，才能启用 WhatsApp 联系方式。如果改过源码或初始化资料，先保存/提交这些修改再合并，不要强制覆盖。回滚时切回已记录的提交，重新安装对应依赖并重启。
...
**本次需要覆盖完整 `app/` 和 `data/site.json` 后重启服务：WhatsApp 链接生成及 `whatsapp-v1` 资料迁移都由 Python 后端执行，只更新模板和 static 不够。** 共享牧场的兔子窝、八个在线席位、套索拉回与途中遭遇老鹰、灰狼也依赖后端模型、HTTP 接口和 SQLite 存档逻辑。下列命令同时更新旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
...
本次新增 `whatsapp-v1` 一次性迁移：如果已有数据库的 `contact.whatsapp` 缺失、为空字符串或只含空白，就补入经营者提供的 `+86 15927146828`；已有非空 WhatsApp 号码保持不变。迁移只补充这个联系方式，不替换其他网站资料、产品或牧场存档，并记录执行标志。之后主动导入空号码或不含 `whatsapp` 的旧版资料时，启动不会再次强行补回；导入内容请保留希望展示的联系方式。
