...
画面以远处浅色坡地、中间明亮草地和近处深色植物分出层次；池塘有深浅、岸边反射与细小水纹，兔子窝和篱笆使用同一方向的暖光与投影。兔子随地图远近轻微缩放，点击位置和套索也按相同尺度对齐，服务器坐标不变。草地纹理和前景植物只在窗口尺寸变化时重新绘制并缓存；每帧只画动物、少量水纹和飘絮。仍保持约 30 帧上限，暂停、后台省电与减少动态效果的设置继续生效，没有新增图片下载、三维引擎或服务器依赖。
