...
- **Lasso**：选 Lasso 后点兔子，抛出套索。按住 **Hold to pull** 将它拉向草地下方的公共篮子，松手停止拉动；**Let go** 解开绳子。至少需要持续拉约 4.5 秒，距离较远的兔子需要更久；进度条显示收回的进度。套住后 25 秒还未入篮，兔子会挣脱。所有人都能看见绳子，只有抛出这根套索的访客能拉动或解开，其他人不能接管。
...
套索相关验证包括拉回需要持续操作、松手和超时、多人争抢、途中遭遇鹰狼，以及状态保存与恢复。公开交互由服务器校验，不能通过旧的捕捉请求绕过拉回过程。
...
**共享牧场版本包含 Python 模型、HTTP 接口和 SQLite 存档逻辑，本次套索拉回与途中遭遇老鹰、灰狼的玩法也需要更新后端。必须覆盖完整 `app/` 并重启服务；只更新模板和 static 不会启用完整功能。** 下列命令同时更新旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
