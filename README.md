...
- **Lasso**：选 Lasso 后点兔子，抛出套索。按住 **Hold to pull** 将它拉向自己席位的篮子，松手停止拉动；**Let go** 解开绳子。至少需要持续拉约 4.5 秒，距离较远的兔子需要更久；进度条显示收回的进度。套住后 25 秒还未入篮，兔子会挣脱。所有人都能看见绳子，只有抛出这根套索的访客能拉动或解开，其他人不能接管。
- **Release**：每次把共享篮子中的一只兔子放回草地。老鹰或灰狼可以在拉回途中叼走兔子，只有真正进入篮子才受到保护；所有访客都能使用这个篮子。
- **Pause / Play**：只暂停自己的画面；暂停时禁用投喂、套索和放回，并停止拉动。切到后台或打开信息面板也停止拉动，绳子的剩余时间仍由公共牧场决定。Play 重新同步到最新状态，其他访客的游玩不会被你的暂停影响。
- **Sync**：立即获取共享牧场的最新状态，不会清空或重置世界。
- **About & contact / Help**：打开个人介绍和联系卡片，或查看玩法；打开面板时停止连续动画，并降低同步频率，关闭后按原来的播放状态恢复。
- **键盘**：聚焦地图后用方向键移动光圈，Enter 或空格使用工具；W / C / N 切换看兔子、胡萝卜和套索。已经套住兔子时，在地图或拉动按钮上按住空格拉动，松开空格停止；聚焦拉动按钮时按 Enter 可切换开始或停止拉动。
...
- **拉绳续接**：按住时浏览器定期续接一段约 1.2 秒的拉动许可，不逐次写入数据库。断网后许可自动到期；服务重启会停止所有正在拉动的绳子，原访客回来后可继续拉动尚未到期的套索。
...
八席位验证覆盖同一网络下八人同时拉回、旁观候位、刷新复用身份、离线席位回收、保留中的套索与重启恢复。套索相关验证包括拉回需要持续操作、松手和超时、多人争抢、途中遭遇鹰狼，以及状态保存与恢复。公开交互由服务器校验，不能通过旧的捕捉请求绕过拉回过程。
...
**共享牧场版本包含 Python 模型、HTTP 接口和 SQLite 存档逻辑，本次八个在线席位、套索拉回与途中遭遇老鹰、灰狼的玩法也需要更新后端。必须覆盖完整 `app/` 并重启服务；只更新模板和 static 不会启用完整功能。** 下列命令同时更新旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
