...
- **Lasso**：选 Lasso 后点击一次兔子，套索会抛出并自动将兔子拉向自己席位的篮子，不需要第二次点击或按住按钮；**Let go** 可以主动解开绳子。自动拉回至少约需 4.5 秒，距离较远的兔子需要更久；进度条显示收回的进度。套住后 25 秒还未入篮，兔子会挣脱。所有人都能看见绳子，只有抛出这根套索的访客能拉动或解开，其他人不能接管。
- **Release**：每次把共享篮子中的一只兔子放回草地。老鹰或灰狼可以在拉回途中叼走兔子，只有真正进入篮子才受到保护；所有访客都能使用这个篮子。
- **Pause / Play**：只暂停自己的画面；暂停时禁用投喂、套索和放回，并停止拉动。切到后台或打开信息面板也停止拉动，回到活动画面后，未过期的套索会自动继续拉回；绳子的剩余时间仍由公共牧场决定。Play 重新同步到最新状态，其他访客的游玩不会被你的暂停影响。
...
八席位验证覆盖同一网络下八人同时拉回、旁观候位、刷新复用身份、离线席位回收、保留中的套索与重启恢复。套索相关验证包括单次抛绳后自动续接、暂停和恢复、取消及超时、多人争抢、途中遭遇鹰狼，以及状态保存与恢复。公开交互由服务器校验，不能通过旧的捕捉请求绕过拉回过程。
