...
现在整个首页都是一个所有访客共同游玩的二维兔子牧场。大家看到同一群兔子跳动、结伴、生出不同毛色的宝宝，也可以放胡萝卜、像牛仔一样用套索套住兔子并拉回共享篮子，还可以放出家犬，让兔子在草地上四散跳开。老鹰和灰狼偶尔来访，拉回途中也可能把兔子叼走，只有进了篮子才安全。一个人的操作会同步给其他访客，刷新或重新进入也会保留牧场进度。画面上保留悬浮工具与统计；右上角只有一个醒目、间歇轻跳的 **About & contact** 按钮，打开简短的个人介绍后，可直接通过 **WhatsApp +86 15927146828** 联系经营者。希望这片小草地，让初次见面多一点轻松。

## 当前网站内容

- **个人介绍**：第一人称说明一人经营的工作方式，放在随时可以打开或关闭的信件面板里。
- **共享兔子牧场**：Canvas 绘制草地、池塘和不同毛色的兔子；服务器统一管理跳动、结伴和繁殖，所有访客共享兔子、胡萝卜、套索与篮子。支持投喂、套住并自动拉回、松绳和放出家犬；页面自动同步，不再显示 Pause、Release 或 Sync 按钮。
- **共享家犬**：点击 Let dog out 后，一只家犬从自己的席位跑出来，驱赶附近的自由兔子；20 秒后回家，所有访客看见同一只狗与同一次追逐。
...
- **业务面板**：个人介绍、WhatsApp 联系按钮与折叠的采购品类集中在一个弹窗里，关闭即可回到牧场。右上角移除了 Field journal 和 Help 入口，只保留 About & contact；系统开启“减少动态效果”时，入口不会跳动。
- **直接联系**：不再展示站内询价表单或消息生成器。买家点 WhatsApp 按钮后，打开与 **+86 15927146828** 的对话，并预填一条可编辑的英文中国商品代购咨询；发送仍由买家自己确认。采购品类中的“Something else?”和产品详情页也直接链接到 WhatsApp。
...
| SQLite（Python 自带 sqlite3） | 持久化网站资料、共享兔子、篮子、胡萝卜、家犬及牧场进度 |
...
初次进入时不默认选中任何工具，Watch 仍保留，选择后才生效。

- **Watch**：看兔子跳动、结伴；选中后点一下兔子会冒出爱心。
...
- **Let dog out**：点击即可从自己的席位放出一只家犬，无需再点地图。家犬按共享时钟活动 20 秒，追赶地面上的自由兔子，让它们跑开；不会抓走或伤害兔子，也不打扰已经套住、进篮或钻窝的兔子。家犬和兔子沿陆地绕开池塘。按钮显示剩余秒数；家犬还在场上时，所有访客都不能再放第二只，20 秒结束后按钮恢复。已占有席位且连接正常时才能放狗，旁观者可以看见同一过程。
- **About & contact**：右上角唯一的业务入口，打开个人介绍和 WhatsApp 联系卡片；打开面板时停止连续动画，并降低同步频率，关闭后恢复游戏画面与正常同步。入口用较醒目的像素色块和间歇轻跳提醒访客，悬停、聚焦或打开面板时停止跳动。
...
兔子使用手工排列的像素精灵：有限色板描绘不同毛色、长耳朵、前后腿和眼睛高光，保留清晰的方格轮廓。静止帧和四个跳跃帧表现预蹲、蹬地、腾空与落地，并有眨眼变化；每种毛色和姿态首次绘制后缓存复用。老鹰、灰狼、家犬和胡萝卜也使用同一套像素绘制方式，放大时关闭平滑处理，不下载模型或大图。显示位置仍由共享快照决定，动画不会自行移动、繁殖或改变兔子。

草地采用分层像素场景：深浅绿树冠围住明亮的块状草坪，点缀少量小花；阶梯状池塘搭配玉绿色水面、石头和浅土色岸边，奶油色篱笆、土路与兔子窝保持统一色板。兔子随地图远近轻微缩放，点击位置和套索也按相同尺度对齐，服务器坐标不变。草地纹理和边缘前景只在窗口尺寸变化时重新绘制并缓存；水面仅有四组缓慢变化的像素水纹。仍保持约 30 帧上限，后台和弹窗省电与减少动态效果的设置继续生效，没有新增图片下载、三维引擎或前后端依赖。

老鹰或灰狼会在随机时间来访：首次约经过 15–30 秒活跃时间，后续间隔约 35–70 秒，具体以模型计时为准。每次先有约 3 秒预警，随后来访者最多带走草地上的一只兔子，以卡通动画表现，没有血腥画面。套索上的兔子仍在草地上，可能在拉回途中被叼走，此时绳子断开、捕捉失败。篮子里的兔子安全；野生动物不会把草地上的兔子减少到两只以下。拉入篮子可以保护兔子，所有席位的捕获数量记录在同一个共享篮子里。旧版 `release` 接口为兼容保留，当前页面不再提供放回按钮。

### 共享状态怎样运行和保存

- **八个在线席位**：草地左右各有四个固定席位，编号与套索颜色一一对应。访客进入后自动占一个空位，可以看见其他席位抛绳、暂时停止拉动、拉回、成功入篮或被鹰狼抢走的过程。兔子拉向玩家自己的席位，所有席位的捕获仍进入同一个共享篮子；没有私人财产或虚构玩家。点击在线人数可以展开每个席位的实时进度。
...
- **唯一状态**：FastAPI 进程以 10 Hz 更新同一个牧场，负责运动、配对、出生毛色、随机来访、家犬驱赶、套索进度和交互结果。浏览器约每 1 秒同步一次，使用平滑绘制显示移动，不自行决定出生、毛色、来访或修改共享数量。所有设备使用同一张固定 1000 × 600 逻辑地图；窗口变化只改变显示投影。
- **及时保存**：在原有 SQLite 数据库内使用 `meadow_state` 表，保存兔子及毛色、共享篮子、胡萝卜、来访事件、家犬与相关进度。套住、解绳及入篮、被叼走、超时挣脱的结果会先保存再同步给访客；拉动中的进度和普通运动每 15 秒及服务正常退出时保存，异常掉电后恢复到最近一次成功保存的位置。旧版存档会自动兼容，无需清空数据库。
- **飞行与捕捉结果**：服务器统一判断套索飞行、落点、命中或扑空，浏览器只绘制轨迹。飞行阶段同样保留席位及目标预约，不能靠旧请求跳过飞行。世界标识与套索编号组成稳定的捕捉记录标识；近期个人结果按原规则最多 12 条、保留 8 秒公共牧场时间。旧图鉴代码与浏览器本地记录保留，当前首页不加载图鉴模块、没有图鉴入口，也不会再记录新印章；没有新增用户表或数据库服务。
- **存档升级**：当前存档为 v7，兼容 v1–v6。家犬字段为旧存档补上“无家犬”的初始状态和家犬编号，保留世界身份、兔子、毛色、篮子、套索、钻窝进度、出口及穿行次数；网站资料不受影响。完整钻窝穿行发生在两次同步之间时，客户端也不会把兔子画成横穿地图。
- **家犬存档与重试**：家犬出现和消失都先写入 SQLite，再公布结果；途中位置与剩余时间沿用 15 秒及正常退出时保存的规则。正常重启后继续剩余的运行时间，异常掉电后以最近成功保存的进度为准，不补算停机时间。每次放狗请求都有身份校验和唯一编号，成功和“家犬已在场”的结果都在最近 1000 个请求回执范围内持久记录；回执仍保留时，重复提交同一请求，包括家犬离开或重启后重试，都不会额外放出一只狗。
- **拉绳续接**：成功套住后，浏览器自动开始拉动，并在活动画面中定期续接一段约 1.2 秒的拉动许可，不逐次写入数据库。断网后许可自动到期；服务重启会停止所有正在拉动的绳子，原访客回到活动画面后会自动继续拉动尚未到期的套索。
- **休息与续接**：通常没人观看 15 秒后暂停服务器计算；如果家犬仍在场，牧场会继续按共享时钟运行，直到这次 20 秒活动结束并保存消失结果，再按无人观看的规则休息。下次有人进入时从当前内存或存档继续，不补算暂停期间的繁殖或野生动物来访。刷新页面或重新进入不会创建新牧场。
- **浏览器省电**：浏览器以约 30 帧/秒为绘制上限并复用背景；切到后台时停止绘制和轮询，打开信息面板时停止连续动画并降低同步频率。回到前台或关闭面板后恢复；未过期的套索也会自动继续拉回。系统开启“减少动态效果”时，每次收到共享状态后静态显示最新画面，不连续播放跳动和装饰动画，但仍能选择工具、投喂、套兔和放狗，不再默认永久暂停。
- **连接提示**：左上角显示连接状态。连接断开时停用投喂、套兔、放狗等共享操作，等待重新连接并同步；界面不会以本地假状态继续更改共享世界。

这些逻辑都运行在现有 FastAPI 服务中，无新增服务、框架、依赖或 Nginx 配置。**Uvicorn 必须保持 1 个 worker**，不能通过多进程或多实例同时写同一牧场。本次家犬功能涉及前端绘制、后端共享模拟、HTTP 接口和 v7 存档，已有服务器必须更新完整 `app/` 并重启，不能只换 HTML 和静态文件。仍从更早版本升级时，一并更新下文的两个 JSON 种子文件，以补齐 WhatsApp 等既有资料迁移。
...
- `contact.whatsapp`：当前公开联系方式为 `+86 15927146828`。使用包含国家区号的号码；启动时去除显示用的空格等分隔符，生成 `https://wa.me/8615927146828` 链接，并附上英文代购咨询。首页和产品详情页以 WhatsApp 为直接联系渠道。
- `contact.email` / `phone` / `wechat` / `address`：为兼容已有资料继续保存在数据库里，本次不会删除或覆盖。当前首页不再引导买家填写表单或发送询价邮件。
...
在云服务器安全组放行 80 和 443；保留已有 SSH 管理端口。8000 只监听本机，无需向公网开放。询价直接跳转到 WhatsApp，由买家确认发送；网站不收集表单或保存询价，也没有付款功能。
...
数据库位于代码目录之外，正常拉取代码不会覆盖它；JSON 主要用于第一次初始化，已有资料的特定升级通过下文的一次性迁移完成。本次需要拉取完整代码并重启，才能启用家犬接口、v7 存档与精简后的工具栏；旧版本所需的 WhatsApp 资料迁移仍会自动执行。如果改过源码或初始化资料，先保存/提交这些修改再合并，不要强制覆盖。回滚时切回已记录的提交，重新安装对应依赖并重启。
...
八席位验证覆盖同一网络下八人同时拉回、旁观候位、刷新复用身份、离线席位回收、保留中的套索与重启恢复。套索相关验证包括飞行期间目标移动、固定落点命中与扑空、提前续接、扑空存档、途中取消与鹰狼抢夺、飞行及旧版存档恢复、个人捕捉结果归属与刷新去重，以及单次抛绳后自动续接、暂停和恢复、取消及超时、多人争抢、途中遭遇鹰狼，以及状态保存与恢复。保留的旧图鉴辅助函数仍有回归测试，当前首页不加载图鉴。公开交互由服务器校验，不能通过旧的捕捉请求绕过拉回过程。联系方式迁移测试验证 WhatsApp 补充、旧格式导入兼容性、重复启动和其他资料及牧场存档保留。

家犬测试验证 20 秒消失、陆地绕行、兔子受惊与不受伤、套住/篮中/洞内兔子保护、进洞与配对交错、所有访客共享、席位和请求校验、重复请求幂等、忙碌结果持久化、正常重启续接、无人观看时完成，以及保存失败回滚和 v1–v6 升级保留。
...
网站资料使用 `site_settings`（品牌和联系方式）与 `products`（按 slug 和显示顺序存储产品）表，详细文本和规格以 JSON 保存在表内。共享牧场另用同一数据库中的 `meadow_state` 表，由服务器自动维护。网站资料没有公开修改接口或管理后台，仍通过服务器终端维护；公开的牧场交互接口用于投喂、套住、拉动、解绳和放出共享家犬；旧 `release` 接口为兼容保留，页面不再展示。套索控制权不能从公开快照中取得，放狗也需要合法访客身份和可用席位。
...
满屏共享牧场右上角只保留一个醒目、间歇轻跳的 **About & contact** 入口，在面板内以简短英文介绍个人代购及商业采购。家居、服饰、电子、户外、乐器、工具、礼品与商业采购是示例方向，并非限制清单。采购分类仍折叠放置，关闭面板即可回到游戏。

面板首先引导买家直接给 **WhatsApp +86 15927146828** 发消息，说明需要什么中国商品。买家可以在 WhatsApp 里发送商品链接、照片、数量及收货国家，经营者再沟通采购、报价和运输安排。网站不再展示询价表单或消息生成器，不上传图片、保存订单或自动发送消息。产品详情页也通过 WhatsApp 发起咨询。

### 从旧版升级（此次不能只更新模板和 static）

**本次必须覆盖完整 `app/` 并重启服务：共享家犬的 20 秒驱赶、HTTP 接口和 v7 存档都由 Python 后端执行，只更新模板和 static 不够。** 页面移除 Pause、Release、Sync 按钮，Watch 初始不选中，新增 Let dog out 按钮及倒计时。原有兔子窝、八个在线席位、套索拉回、鹰狼事件和 WhatsApp 联系方式保留；没有新增依赖。下列命令同时更新旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
...
既有 `whatsapp-v1` 一次性迁移继续生效：如果已有数据库的 `contact.whatsapp` 缺失、为空字符串或只含空白，就补入经营者提供的 `+86 15927146828`；已有非空 WhatsApp 号码保持不变。迁移只补充这个联系方式，不替换其他网站资料、产品或牧场存档，并记录执行标志。之后主动导入空号码或不含 `whatsapp` 的旧版资料时，启动不会再次强行补回；导入内容请保留希望展示的联系方式。

重启后服务会自动创建或读取 `meadow_state` 共享存档，并将 v1–v6 升级到 v7；旧存档补充家犬字段且初始没有家犬，已保存的活动家犬继续剩余运行时间。更早的存档仍会自动兼容套索、钻窝、毛色与随机来访状态；已有兔子的毛色、共享篮子和出生统计保留，新生兔子仍随机获得毛色，无需清空数据库或重开牧场。网站的现有品牌、介绍、联系方式和产品资料保留。原依赖、systemd 与 Nginx 配置无需变更，但须确认 Uvicorn 仍只有 1 个 worker。静态资源版本已更新，刷新浏览器即可加载新画面。
