...
这些逻辑都运行在现有 FastAPI 服务中，无新增服务、框架、依赖或 Nginx 配置。**Uvicorn 必须保持 1 个 worker**，不能通过多进程或多实例同时写同一牧场。家犬功能涉及前端绘制、后端共享模拟、HTTP 接口和 v7 存档，从尚无家犬的版本升级时必须更新完整 `app/` 并重启，不能只换 HTML 和静态文件。仍从更早版本升级时，一并更新下文的两个 JSON 种子文件，以补齐 WhatsApp 等既有资料迁移。
...
**从家犬功能之前的版本升级时，必须覆盖完整 `app/` 并重启服务：共享家犬的 20 秒驱赶、HTTP 接口和 v7 存档都由 Python 后端执行，只更新模板和 static 不够。** 当前页面移除 Pause、Release、Sync、Watch 按钮及左上角计分板，底部只保留 Carrot、Lasso、Let dog out 三个按钮及家犬倒计时；工具可再次点击取消，未选工具时点击兔子会冒出本地爱心。原有兔子窝、地图上的八个席位和篮子、套索拉回、鹰狼事件和 WhatsApp 联系方式保留；没有新增依赖。本次工具栏调整不改变存档格式或数据。下列命令更新完整代码和旧版本可能需要的两个 JSON 种子文件，适用于项目位于 `/opt/personal-shopper` 的服务器。按用户偏好不备份旧代码，保留数据库、环境配置及额外图片：
