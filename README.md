...
新版首页增加“下一站，由你决定”：三个兴趣世界 × 三个经验阶段，共九种编辑策划的小任务。选择会与 3D 展厅双向联动，并生成一张配色不同的探索卡。随机探索会选择与当前不同的世界。

- 探索卡可下载为 SVG 图片，采用系统字体、可放大，部分图片预览软件对中文字体显示可能不同。
- “记住这次选择”仅将兴趣和经验阶段存入访客浏览器 localStorage；可以用“重新出发”清除。预算和咨询正文不保存，也不上传。
- 选择预算后可展开、编辑并复制咨询清单，通过现有联系方式自行发送。没有自动发送、订单、库存承诺或价格推荐。
- HTTP 或不支持剪贴板的环境会选中清单文字，提示访客手动复制。
- 探索任务和文案在 `app/static/adventure-data.js`；不是在 SQLite 中维护。网站品牌、产品与联系方式继续来自数据库。
- 不需要增加 Python 依赖或迁移数据库，沿用更新模板、静态资源后重启服务的方式。脚本使用 `.js` 扩展名，兼容现有 Nginx JavaScript MIME 配置。

验证：`node --test tests/adventure.test.mjs`（仅开发测试使用 Node，生产不需要）。覆盖九种组合、咨询内容和失效本地记录处理。网站与 SQLite 测试命令保持不变。
