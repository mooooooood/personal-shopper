...
- **飞行与图鉴**：服务器统一判断套索飞行、落点、命中或扑空，浏览器只绘制轨迹。飞行阶段同样保留席位及目标预约，不能靠旧请求跳过飞行。存档升级为 v5，兼容 v1–v4，原有兔子、篮子、拉回进度及资料都保留。世界标识与套索编号组成稳定的捕捉记录标识，重启不会重复盖章；近期个人结果按原规则最多 12 条、保留 8 秒公共牧场时间。图鉴为浏览器本地最多 8 枚印章，没有新增用户表或数据库服务。
