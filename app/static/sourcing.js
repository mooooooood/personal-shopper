...
byId('prepare-request').addEventListener('click',()=>{
 ...
 byId('request-result').hidden=false;status.textContent='Your enquiry is ready. Edit or copy it, then send it using my contact details.';byId('request-message').focus();
});
byId('copy-request').addEventListener('click',async()=>{
 try{await navigator.clipboard.writeText(byId('request-message').value);status.textContent='Copied. Send the message to me when you’re ready.';}
